...
const char * const BITCOIN_CONF_FILENAME = "bleumintcoin.conf";
const char * const BITCOIN_PID_FILENAME = "bleumintcoin.pid";
...
#else
    const char* pszModule = "bleumintcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bleumintcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bleumintcoin";
#else
    // Unix
    return pathRet / ".bleumintcoin";
#endif
#endif
}
