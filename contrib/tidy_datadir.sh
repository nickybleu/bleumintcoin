...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bleumintcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bleumintcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bleumintcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bleumintcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Bleumintcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bleumintcoin 0.8 datadir."
    ;;
esac
