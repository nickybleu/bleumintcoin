Bleumintcoin Core version 0.13.2 is now available from:

  <https://download.bleumintcoin.org/bleumintcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/bleumintcoin-project/bleumintcoin/issues>
...
updates will be released anymore. Without security updates, using a bleumintcoin
...
ECDSA signatures inside Bleumintcoin transactions now use validation using
...
Previous versions of Bleumintcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bleumintcoin Core was to raise the default minimum
relay fee.

Bleumintcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Bleumintcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running bleumintcoind without having to do any manual configuration.
...
Bleumintcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bleumintcoin Core will relay transactions with
...
entirely in Bleumintcoin Core 0.13, and it is at this time undecided whether the
...
Bleumintcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Bleumintcoin Core automatically creates a hidden service to listen on, without
manual configuration. Bleumintcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Bleumintcoin Core is listening, and
...
Bleumintcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Bleumintcoin
Core to use floating fees. By default, Bleumintcoin Core will use floating
fees.
...
At all times, Bleumintcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Bleumintcoin Core will never create transactions paying less than
...
`bleumintcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bleumintcoinrpc>
        ...
        # optional bypass bleumintcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< bleumintcoinrpc:<password>
    </Location>
...
Restarting bleumintcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in bleumintcoin.conf
...
bleumintcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bleumintcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Bleumintcoin Core code base has
...
- `bleumintcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `bleumintcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Bleumintcoin scripting system that in combination with
...
Bleumintcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Bleumintcoin Core.
...
The two phases are distinct in the Bleumintcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bleumintcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bleumintcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bleumintcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bleumintcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bleumintcoins being spent is exactly
...
- **Increased security for multisig:** Bleumintcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Bleumintcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bleumintcoin's current ledger (called the UTXO set),
  ...
  note that Bleumintcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bleumintcoin users to individually opt-in to almost any change in the Bleumintcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Bleumintcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Bleumintcoin
client to Bleumintcoin Core 0.13.2 to signal for segwit support. When a
...
Since Bleumintcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `bleumintcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bleumintcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
